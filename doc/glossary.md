...
| **ops-recipe-runner** | このリポジトリ全体。エージェント定義・前処理コア・DB環境を含む、レシピを実行するための一式。レシピ・テンプレSQLの実体は持たない（Backlogが正） | 実装済み |
| **前処理コア** | `src/lib/`（＋`src/script/preprocess/`のCLI）。CSV解析・宣言的lookup実行**のみ**を担う。レシピ解釈・SQL組み立ては行わない（それはClaude Agentの責務） | 実装済み |
| **Local DB Container** | 任意のdumpからMySQLを起動するDockerコンテナ（`docker-compose.yml`の`db`サービス）。クエリの中身は一切知らない受動的なDB提供元。前処理コア（`lookup.ts`/`db.ts`）が直接接続してクエリを実行する | 実装済み |
...
| **handler** | プレースホルダーの埋め方の種別。`agent-literal`（Agentが直接埋める）／`preprocess-csv`（前処理コアがCSVから抽出）／`preprocess-lookup`（前処理コアがDBからlookup） |
...
| 用語 | 意味 |
| --- | --- |
| **ops-recipe-runner（スキル）** | `.claude/skills/ops-recipe-runner/` のルータースキル。Hostの意図を4つに振り分ける入口。`~/.claude/skills/` へリンクすると任意のディレクトリから起動できる |
...
| エージェント | 役割 |
| --- | --- |
| **recipe-requirements-analyst** | 要件ヒアリング。実装前の要件シート作成 |
| **recipe-designer** | 処理設計。手作業SQLをテンプレ＋プレースホルダー宣言＋lookup宣言へ写像 |
| **recipe-implementer** | 前処理コード実装（必要時のみ）。まずコードを増やさない |
| **recipe-tester** | 前処理コアの単体・結合テスト |
| **recipe-documenter** | ①Agent向けレシピ ②運用ドキュメントの作成。Backlog登録 |
| **recipe-reviewer** | 最終ゲート。横断の一貫性・重複・必要十分性・データ安全性の二重チェック |

## 将来像（未着手・構想）

現地の実際の構成（IDE + `.claude/skills/` + `dev/`）に組み込む将来計画。詳細は今後
`doc/architecture/` に追記予定。

| 用語 | 意味 | 状態 |
| --- | --- | --- |
| **orchestration.skill** | 全体統括のみを担うスキル。現状の`ops-recipe-runner`から分割される可能性がある | 構想 |
