# チケット作成ワークフロー（レシピ実行シナリオ）

> 本書は**既存レシピからチケット(SQL)を生成・起票する「実行」シナリオ**。
> 新しいレシピを**作成**するシナリオは `doc/architecture/recipe-creation.md`（別物・担当はドキュメンター）。
> 確認済み範囲は `doc/test-summary.md` を参照。
...
| 前処理コア | 宣言に基づく汎用 lookup 実行（read-only）。SQLテンプレは持たない |
...
| データ由来 | `{{orderIds}}` `{{salesIds}}` | 前処理コア | CSV列 / lookup宣言 |
...
祝日・営業日の扱いは間違いやすいため、Agent が日付を出したら次を必ず確認する:
